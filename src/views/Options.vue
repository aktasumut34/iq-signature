<template>
  <div class="options">
    <el-form label-position="left" label-width="150px">
      ...
      <el-form-item label="Socials text color">
        <el-color-picker
          ref="colorPicker3"
          v-model="socialsTextColor"
          @active-change="onPreviewSocialsTextColor"
        />
      </el-form-item>
...
export default {
  ...
  computed: {
    ...
    socialsTextColor: {
      get() {
        return this.options.color.socialsText;
      },
      set(v) {
        this.$store.dispatch("updateOptions", { color: { socialsText: v } });
        this.$store.commit("SET_COLOR", { socialsTextPreview: undefined });
      },
    },
    fontSize: {
      get() {
  ...
  watch: {
    ...
    "options.color.socialsText"(v) {
      if (v === null) this.$store.commit("SET_COLOR", { socialsText: "#fff" });
    },
  },
  ...
  methods: {
    ...
    onPreviewSocialsTextColor(v) {
      this.$store.commit("SET_COLOR", { socialsText: v });
    },
    resetPreviewColor() {
      this.$store.commit("SET_COLOR", { mainPreview: undefined });
      this.$store.commit("SET_COLOR", { secondaryPreview: undefined });
    ...
    onCloseColorPicker() {
      ...
      this.$refs.colorPicker3.$children[0].$on("input", (e) => {
        if (!e) this.resetPreviewColor();
      });
    },
  },
